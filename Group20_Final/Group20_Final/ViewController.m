...
- (void)viewDidLoad {
    ...
    _segment = [[UISegmentedControl alloc] initWithItems:@[@"Diary", @"Calendar", @"Write"]];
    _segment.tintColor = ThemeFontColor;
...
- (NSMutableArray *)arrayTodayDics {
    if (!_arrayTodayDics) {
        ...
        for (int i = -2; i < 3; i++) {
            ...
            [_arrayTodayDics insertObject:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%ld", components.year], @"year", MONTH[components.month - 1], @"month", [NSString stringWithFormat:@"%ld", (long)components.day], @"day", WEEKDAY[components.weekday - 1], @"week", date, @"date", nil] atIndex:i + 2];
        }
    }
...
- (NSDictionary *)getDayDicAfterDays:(int)days fromDate:(NSDate *)date {
    ...
    NSDateComponents *components = [calendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday fromDate:date];
    NSDictionary *dicDay = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%ld", components.year], @"year", MONTH[components.month - 1], @"month", [NSString stringWithFormat:@"%ld", (long)components.day], @"day", WEEKDAY[components.weekday - 1], @"week", date, @"date", nil];
    return dicDay;
}
