...
- (void)viewDidLoad {
    ...
    [_navigationBar setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"bgd"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DWSettingSwitchTableViewCell *cell = [[DWSettingSwitchTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SettingSwitchCell" title:@"Open password"];
    cell.switcher.tag = 0;
    cell.delegate = self;
    return cell;
}
...
- (void)saveKeyIntoDataBase:(NSString *)key {
    if (key) {
        ...
        [self updateUserInfoIntoDataBase:userInfo completion:^{
            [SettingManage sharedInstance].isLocked = YES;
        }];
    }
}
