...
- (UITextView *)tVDetail {
    if (!_tVDetail) {
        ...
        _tVDetail.layer.backgroundColor = [[UIColor colorWithRed:1 green:1 blue:1 alpha:0.2 ]CGColor];
        
        _tVDetail.layer.cornerRadius = 10;
        [_tVDetail.layer setMasksToBounds:YES];
...
- (UIView *)toolBar {
    if (!_toolBar) {
        ...
        _buttonEmotion.frame = CGRectMake(10, _toolBar.frame.size.height / 2 - 25 / 2, 30, 30);
        _buttonWeather.frame = CGRectMake(CGRectGetMaxX(_buttonEmotion.frame) + 10, _buttonEmotion.frame.origin.y, 30, 30);
        _buttonSave.frame = CGRectMake(_toolBar.frame.size.width - 10 - 50, _toolBar.frame.size.height / 2 - 30 / 2, 35, 35);
...
- (void)buttonSaveTap {
    ...
    if ([self.tFTitle.text isEqualToString:@""] && [self.tVDetail.text isEqualToString:@""]) {
        self.tipsView.message = @"Empty diary";
